...
function ContinentMap({ activeId, onSelect }) {
  const nc = { cardea:"#2A5F9E", silvana:"#1A6B4A", mograheim:"#8B6914", riet:"#4A7A2E", karansa:"#B85C2A", valhart:"#8B2D2D" };
  const pois = [
  ...
  const R = (id,d,f,s,sw,cl) => <path key={id} d={d} fill={activeId===id?f.replace(")",",0.22)").replace("rgb","rgba"):f} stroke={s} strokeWidth={sw} style={{cursor:"pointer"}} onClick={()=>onSelect(id)}/>;
  return (
...
function LocModal({loc,color,onClose}) {
  if(!loc) return null;
  return <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center",padding:20,cursor:"pointer"}}>
...
export default function NationsWiki() {
  ...
  const n=nations.find(x=>x.id===activeId);
  ...
  useEffect(()=>{if(ref.current)ref.current.scrollTop=0;setShowNav(false);},[activeId]);

  const Sec=({title,children})=><div style={{marginBottom:"2.5rem"}}><h3 style={{fontFamily:"'Noto Serif KR',Georgia,serif",fontSize:"15px",fontWeight:600,letterSpacing:"0.12em",borderBottom:`2px solid ${n.color}`,paddingBottom:6,marginBottom:14,color:n.color}}>{title}</h3>{children}</div>;
  const Prose=({text})=><div style={{fontSize:"14px",lineHeight:1.85,color:"#3a3a3a"}}>{text.split("\n\n").map((p,i)=><p key={i} style={{marginBottom:12}}>{p}</p>)}</div>;

  return <div style={{display:"flex",height:"100vh",fontFamily:"'Noto Sans KR',sans-serif",background:"#F7F4EE",color:"#2a2a2a"}}>
